  ...
  <div class="container">
    <div class="row">
      <div class="col-md-12 mt-5 mb-5 d-block">
        <div class="row mb-3">
          <div class="col-md-4">
            <h3 class="float-left">Classes</h3>
          </div>
          <div class="col-md-8">
...
export default {
  ...
  created() {
    ...
    const userClassesRecord = localStorage.getItem("userClassesRecord");
    if (classes.length > 0) {
      let newRecord = [];
      let userId = localStorage.getItem("userId");
      let userRecord = userClassesRecord ? JSON.parse(userClassesRecord) : [];

      userRecord = userRecord.filter(record => record.userId === userId);
      if (userRecord.length > 0) {
        for (let i = 0; i < classes.length; i++) {
          let findActivity = userRecord.findIndex(
            record => record.classId === classes[i].id
          );
        ...
        let newRecordsToBePushed = classes.map(act => {
          return {
            id: uuid.v4(),
            userId: userId,
            classId: act.id,
      ...
      localStorage.setItem("userClassesRecord", JSON.stringify(newUserRecord));
      this.classes = newUserRecord;
  ...
  methods: {
    search(e) {
      e.preventDefault();
      if (!this.searchText) return (this.viewable = this.classes);
    ...
    saveReview(e) {
      e.preventDefault();
      let record = JSON.parse(localStorage.getItem("userClassesRecord"));
      ...
      localStorage.setItem("userClassesRecord", JSON.stringify(record));
    },
    sortByTopic() {
      let sorted = [];
